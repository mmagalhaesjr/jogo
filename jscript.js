...
function criaExplosao(tipo,x,y){ //Tipo 1=AR, 2=TERRA
	...
	if(tipo==1){
		...
		att2.value="top:"+y+"px;left:"+x+"px;";
		att4.value="img_gif/explosao_ar.gif?"+new Date();
	}else{
		att1.value="explosaoChao";
		att2.value="top:"+(tamTelaH-57)+"px;left:"+(x-17)+"px;";
		att4.value="img_gif/explosao_chao.gif?"+new Date();
	}
...
function gerenciaGame(){
	...
	if(contBombas<=0){
		...
		telaMsg.style.backgroundImage="url('img_gif/vitoria.png') ";
		telaMsg.style.display="block";
	}
	if(vidaPlaneta<=0){
		jogo=false;
		clearInterval(tmpCriaBomba);
		telaMsg.style.backgroundImage="url('img_gif/derrota.png')";
		telaMsg.style.display="block";		
	}
}
//FUNÇÕES DE CONTROLE----------------------------------------
...
function inicia(){
	...
	telaMsg.style.width="700px";
	telaMsg.style.height="500px";
